...
async function loadData() {
  ...
  // 小黑盒：单一收藏来源，直接并入
  xhh.forEach((i) => { if (i?.id) map.set(i.id, { ...i, sources: ['collect'] }); });
...
function applyFilter() {
  ...
  state.filtered = state.all.filter((it) => {
    ...
    if (state.view === 'xiaoheihe' && it.platform !== 'xiaoheihe') return false;
...
/* ---------------------------------------------------------------------------
 * Library 管理（添加书 / 上传封面）——权限 = 持有 PAT（只存在本机浏览器）
 * ------------------------------------------------------------------------- */
const b64encode = (str) => btoa(unescape(encodeURIComponent(str)));

async function ghContents(path, pat, method = 'GET', body = null) {
...
async function dispatchLibraryRebuild(pat) {
  const res = await fetch(`${API_BASE}/actions/workflows/library.yml/dispatches`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${pat}`, Accept: 'application/vnd.github+json', 'Content-Type': 'application/json' },
    body: JSON.stringify({ ref: CONFIG.BRANCH }),
  });
  if (!res.ok) throw new Error(`触发重建失败 HTTP ${res.status}`);
}

function openLibManage() {
  const pat = localStorage.getItem(PAT_KEY);
  if (!pat) { openPatModal(); showToast('管理需要管理员 Token，请先配置', true); return; }
  const sel = $('#libPickBook');
  sel.innerHTML = state.library.map((b) =>
    `<option value="${escapeHtml(b.query || b.title)}">${escapeHtml(b.title)}</option>`).join('');
  $('#libAddTitle').value = '';
  $('#libPickFile').value = '';
  $('#libManageModal').showModal();
}

async function addLibraryBook() {
  const title = $('#libAddTitle').value.trim();
  if (!title) return showToast('请输入书名', true);
  const pat = localStorage.getItem(PAT_KEY);
  if (!pat) return showToast('需要管理员 Token', true);
  const btn = $('#libAddBtn');
  btn.disabled = true; btn.textContent = '提交中…';
  try {
    const data = await ghContents('data/library.json', pat);
    const lib = JSON.parse(atob(data.content));
    if (lib.some((i) => i.query === title)) { showToast('这本书已在书单中', true); return; }
    lib.push({ query: title });
    await ghContents('data/library.json', pat, 'PUT', {
      message: `add book: ${title}`,
      content: b64encode(JSON.stringify(lib, null, 2)),
      sha: data.sha,
    });
    await dispatchLibraryRebuild(pat);
    showToast('已提交，云端抓取中（约 1~2 分钟）');
    $('#libManageModal').close();
  } catch (err) {
    showToast('添加失败：' + err.message, true);
  } finally {
    btn.disabled = false; btn.textContent = '添加并重建';
  }
}

async function uploadBookCover() {
  const query = $('#libPickBook').value;
  const file = $('#libPickFile').files[0];
  if (!query) return showToast('请选择书', true);
  if (!file) return showToast('请选择图片', true);
  const pat = localStorage.getItem(PAT_KEY);
  if (!pat) return showToast('需要管理员 Token', true);
  const btn = $('#libUploadBtn');
  ...
  try {
    ...
    const realPath = `assets/img/books/${query}.jpg`;
    let sha = null;
    try { const exist = await ghContents(realPath, pat); sha = exist.sha; } catch { /* 新文件 */ }
    await ghContents(realPath, pat, 'PUT', { message: `cover: ${query}`, content: base64, sha });
    // 给这本书写入 coverOverride
    const data = await ghContents('data/library.json', pat);
    const lib = JSON.parse(atob(data.content));
    const item = lib.find((i) => i.query === query);
    if (item) item.coverOverride = realPath;
    await ghContents('data/library.json', pat, 'PUT', {
      message: `cover override: ${query}`,
      content: b64encode(JSON.stringify(lib, null, 2)),
      sha: data.sha,
    });
    await dispatchLibraryRebuild(pat);
    showToast('封面已上传，云端重建中');
    $('#libManageModal').close();
  } catch (err) {
    showToast('上传失败：' + err.message, true);
  } finally {
    btn.disabled = false; btn.textContent = '上传封面';
  }
}

$('#libManageBtn').addEventListener('click', openLibManage);
$('#libAddBtn').addEventListener('click', addLibraryBook);
$('#libUploadBtn').addEventListener('click', uploadBookCover);
$('#libManageClose').addEventListener('click', () => $('#libManageModal').close());
